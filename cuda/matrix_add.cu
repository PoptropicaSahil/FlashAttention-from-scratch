...
void test_matrix_add(int NUM_ROWS, int NUM_COLS, int ROWS_block_size, int COLS_block_size) 
{
    ...
    // May not always be a nice multiple so we take ceiling
    ...
    dim3 grid(num_blocks_COLS, num_blocks_ROWS, 1); // grid = how many blocks we have is a tuple
    dim3 block(COLS_block_size, ROWS_block_size, 1); // block = how many threads we want in each block
    
    // Run the kernel
    // This is the launch grid
